...
export function getLastAssistantText(messages: { role: string; content: { type: string; text?: string }[] }[]): string {
...
export function formatToolCall(call: ToolCall): string {
  const args = call.args && typeof call.args === "object" ? (call.args as Record<string, unknown>) : undefined;
