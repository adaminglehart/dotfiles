...
function buildFooter(ctx: FooterContext, width: number): [string, string, string] {
  ...
  const line1 = line1Segments.join(separator);

  // Line 2: model info (left) and metrics (right)
  ...
  const line2LeftWidth = visibleWidth(line2LeftContent);
  const line2RightContent = line2RightSegments.join(separator);
  const line2RightWidth = visibleWidth(line2RightContent);

  // Build line 2 with padding
  const line2Padding =
    width -
    line2LeftWidth -
    line2RightWidth -
    (line2RightSegments.length > 0 ? separator.length : 0);

  let line2 = line2LeftContent;
  if (line2RightContent) {
    const padding = Math.max(0, line2Padding);
  ...
  function calculateSessionCost(ctx: any): number {
    ...
    try {
      ...
      for (const entry of entries) {
        if (
          entry.type === "message" &&
          (entry as any).message?.usage?.cost?.total
        ) {
